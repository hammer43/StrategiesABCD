...
STATE_FILE  = '/root/gold-signals/pipeline/state_b3.json'
PROCESSED_FILE = '/root/gold-signals/pipeline/processed_b3.json'
TRADES_FILE = '/root/gold-signals/pipeline/trades_b3.json'
                ...
                if price >= p['tp1']:
                    ...
                    for entry_info in entries:
                        ...
                        if entry_dir == 'wait_drop':
                            # State machine: track pullback_low in zone
                            if p['zone_bot'] <= price <= p['zone_top']:
                                p['in_zone'] = True
                                p['pullback_low'] = min(p.get('pullback_low', 9999), price)
                                save_state()
                            pullback_low = p.get('pullback_low', 9999)
                            if p.get('in_zone') and pullback_low < 9999 and price > pullback_low + 0.5:
                                try:
                                    import json as _c1j
                                    c1m = _c1j.load(open('/root/gold-signals/pipeline/candles/1m.json'))
                                    last = c1m[-1] if c1m else None
                                    bullish = last and last['close'] > last['open']
                                except:
                                    bullish = True
                                if bullish:
                                    filled_layers.add(eid)
                                    open_trade(entry_info, p, 'bounce_from_low')
                            # Timeout fallback: 2 minutes elapsed, zone was touched, price still valid
                            try:
                                from datetime import datetime as _fdt, timezone as _ftz
                                sig_time = _fdt.fromisoformat(p["time"].replace("Z","+00:00"))
                                elapsed = (_fdt.now(_ftz.utc) - sig_time).total_seconds()
                            except:
                                elapsed = 0
                            if elapsed > 120 and price <= p["zone_top"] + 1 and price > p.get("adj_sl", 0):
                                filled_layers.add(eid)
                                open_trade(entry_info, p, "fallback_limit")
                                send_telegram(f"[{LABEL}] FALLBACK LIMIT after {int(elapsed)}s")
                            if price < p['zone_bot'] - 0.1:
                                p['entry_direction'] = 'wait_rise'
                                for e in entries:
                                    e['price'] = p['zone_bot']
                                save_state()
                            if price < p.get('adj_sl', 0) - 0.5:
                                account['pending'] = None
                                save_state()
                                send_telegram(f"[{LABEL}] INVALIDATED below SL")
                                break
