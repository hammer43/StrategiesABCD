...
from confluence_engine import get_confluence_score
...
FILL_BUFFER      = 0.15
STATE_FILE       = '/root/gold-signals/pipeline/state_d.json'
PROCESSED_FILE = '/root/gold-signals/pipeline/processed_d.json'
TRADES_FILE      = '/root/gold-signals/pipeline/trades_d.json'
    ...
    while True:
        try:
            ...
            for signal in queue:
                ...
                # Confluence scoring (Strategy D addition)
                confluence = get_confluence_score(signal, price or 0)
                conf_score = confluence["confluence_score"]
                # Boost confidence with confluence
                boosted_confidence = round((score["confidence"] * 0.7) + (conf_score * 100 * 0.3), 1)
                score["confidence"] = boosted_confidence
                score["confluence"] = confluence

                # Strategy C: skip only if truly bad
                # Bearish = half size, not skip
                if score["confidence"] < 50 and htf_bias == 'bearish':
                    send_telegram(
...
async def daily_report():
    while True:
        ...
        with open(f'/root/gold-signals/pipeline/report_d_{today}.json', 'w') as f:
...
client = TelegramClient(
    '/root/gold-signals/pipeline/gold_session_d',
    api_id, api_hash
)
...
async def main():
    ...
    send_telegram(
        f'[{LABEL}] STRATEGY D LIVE\n'
        f'Optimised rules:\n'
        f'- Bearish 4H = half size\n'
        f'- Re-entry = 1.5x size\n'
        f'- Sweep = score boost\n'
        f'- Hybrid ML + FVG + OB + ADM\n'
