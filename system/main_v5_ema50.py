...
STATE_FILE  = f'{BASE_DIR}/state_a_v5.json'
TRADES_FILE = f'{BASE_DIR}/trades_a_v5.json'
SESSION     = f'{BASE_DIR}/gold_session_v5'
                ...
                if not trade.get('tp1_hit') and price <= trade['sl']:
                    try:
                        ...
                        if tp1_first:
                            ...
                            trade['pnl']      = trade.get('pnl', 0) + profit
                            save_trade(trade)
                            save_state()
                            send_telegram(
                                f'[{LABEL}] TP1 50% (candle-recovered) - {trade["id"]}\n'
                                f'+{pips} pips | +${profit}\n'
...
async def handler(event):
    ...
    try:
        ...
        if entry < ema50:
            send_telegram(f'[{LABEL}] SKIP — below EMA50 {ema50}')
            return
